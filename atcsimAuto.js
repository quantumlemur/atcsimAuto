...
if (document.title == 'Atlanta Hartsfield-Jackson Intl.') {
	...
	runwayNW = '27'
	runwaySW = '27'
} else if (document.title == 'London Heathrow') {
	runwayNE = '9L'
	runwaySE = '9L'
	runwayNW = '27R'
	runwaySW = '27R'
} else if (document.title == 'Osaka Kansai Intl.') {
	runwayNE = '6L'
	runwaySE = '6R'
	runwayNW = '24R'
	runwaySW = '24L'
	...
	runwaySW = '30L'
	finalOffset = -200
} else if (document.title == 'Rio de Janeiro RJ (Brazil) Galeao Intl.') {
	runwayNE = '10'
	runwaySE = '15'
	runwayNW = '28'
	runwaySW = '33'
	finalOffset = -250
} else if (document.title == 'St. Louis MO (USA) Lambert Intl.') {
	runwayNE = '11'
	runwaySE = '6'
	runwayNW = '24'
	runwaySW = '30R'
...
calcLines = function() {
	...
	runNreciprocal = !eastFlow ? runwayNE : runwayNW
	runSreciprocal = !eastFlow ? runwaySE : runwaySW
	...
	waypointList = [
		['NDWND', 2, eastFlow?lineX/2:lineX*1.5, northY+(eastFlow?1:-1)*(window.finalOffset!=undefined?finalOffset:0)],
		['SDWND', 2, eastFlow?lineX/2:lineX*1.5, southY+(eastFlow?1:-1)*(window.finalOffset!=undefined?finalOffset:0)],
...
checkDepartures = function() {
	...
	if (!waitingPlane && (!takingOffPlane || G_objPlanes[takingOffPlane][6]>10)) {
		for (let i=0; i<planes.length; i++) {
			...
			if(!p.leg && !p['runway'] && p[16] == 'D') {
				...
				let runwayLR = runway[runway.length - 1]
				let recip = ''
				if (runwayLR == 'L') {
					runwayLR = 'R'
					recip = +runway.substring(0, runway.length-1)
				} else if (runwayLR == 'R') {
					runwayLR = 'L'
					recip = +runway.substring(0, runway.length-1)
				} else {
				...
				recip = (recip+18) + runwayLR
				if (!runwayQueues[runway] || runwayQueues[runway].length==0 || runwayQueues[runway][0].dist > minLandingGap) {
					if (!runwayQueues[recip] || runwayQueues[recip].length==0 || runwayQueues[recip][0].dist > minLandingGap*2) {
						p.leg = 'waiting'
...
spacePlanes = function() {
	...
	if (northQueue.length>0 && G_objPlanes[northQueue[0]].leg != 'approach') {
		northQueue.splice(0, 1)
	}
	if (southQueue.length>0 && G_objPlanes[southQueue[0]].leg != 'approach') {
		...
		if (p.leg == 'approach') {
			...
			if (sequence == 0) {
				...
				if (dist < waypointPrecision) {
					...
					setNav(plane, (p.north?'NDWND':'SDWND'))
					if (p.north) {
						northQueue.splice(0, 1)
					} else {
						southQueue.splice(0, 1)
				...
				desiredPathLength = incomingSpacing*(p.gap?2:1) + (p.north?G_objPlanes[northQueue[sequence-1]].desiredPathLength:G_objPlanes[southQueue[sequence-1]].desiredPathLength)

				let minDist1 = 9999999
				let minDist2 = 9999999
				if (p.lastLeg != undefined) {
					// first the current leg
					let Xstep = (Xvertices[p.north?0:1][p.lastLeg+1] - Xvertices[p.north?0:1][p.lastLeg]) / spacingSteps
					let Ystep = (Yvertices[p.north?0:1][p.lastLeg+1] - Yvertices[p.north?0:1][p.lastLeg]) / spacingSteps
					for (let i=0; i<spacingSteps; i++) {
						let xi = Xvertices[p.north?0:1][p.lastLeg] + Xstep*i
						let yi = Yvertices[p.north?0:1][p.lastLeg] + Ystep*i
						let dist = Math.sqrt(Math.pow(xi-(p[2]+24), 2) + Math.pow(yi-(p[3]+63), 2))
						if (dist > minDist1) {
							break
						}
						minDist1 = dist
					}
					// then the previous leg
					Xstep = (Xvertices[p.north?0:1][p.lastLeg+1+1] - Xvertices[p.north?0:1][p.lastLeg+1]) / spacingSteps
					Ystep = (Yvertices[p.north?0:1][p.lastLeg+1+1] - Yvertices[p.north?0:1][p.lastLeg+1]) / spacingSteps
					for (let i=0; i<spacingSteps; i++) {
						let xi = Xvertices[p.north?0:1][p.lastLeg+1] + Xstep*i
						let yi = Yvertices[p.north?0:1][p.lastLeg+1] + Ystep*i
						let dist = Math.sqrt(Math.pow(xi-(p[2]+24), 2) + Math.pow(yi-(p[3]+63), 2))
						if (dist > minDist2) {
							break
						}
					...
					let minDist = Math.min(minDist1, minDist2)
					if (minDist < 100) {
						numPointsAhead = Math.max(Math.floor((90-minDist) / 5), 0)
						p.onCourse = true
					}
					p.minDist = minDist
				}
			...
			setHeading(plane, eastFlow?(p.north?'105':'075'):(p.north?'255':'285'))
			setSpeed(plane, 240)
			...
			if (p[16]=='A') { // if don't have a leg assigned but we're an arrival (haven't been routed), then insert us into the approach sequence
				...
				if (!p.gapped) {
					if (G_arrNavObjects[p[12]][0] == runwayNE || G_arrNavObjects[p[12]][0] == runwayNW) {
					...
					} else if (G_arrNavObjects[p[12]][0] == runwaySE || G_arrNavObjects[p[12]][0] == runwaySW) {
